      ...
      for (const pattern of DATE_FORMAT_PATTERNS) {
        if (pattern.test(valueStr)) {
          return createIssue(this, {
            message: `文本型日期：单元格 ${cell.address} 包含文本格式的日期，日期函数可能无法正确计算`,
    ...
    if (cell.dataType === 'date' || (cell.dataType === 'text' && cell.value)) {
      ...
      if (ambiguousMatch) {
        ...
        if (firstNum <= 12 && secondNum <= 12 && firstNum !== secondNum) {
          return createIssue(this, {
            message: `日期格式歧义：${valueStr} 可能被解释为 ${firstNum}月${secondNum}日 或 ${secondNum}月${firstNum}日`,
            cellAddress: cell.address,
            sheetName: context.name,
            confidence: ConfidenceLevel.MEDIUM,
            currentValue: valueStr,
            suggestion: '建议使用明确的日期格式如 YYYY-MM-DD 或显示完整月份名称',
            details: {
  ...
  checkExcel(cell, context) {
    ...
    if (cell.numberFormat && String(cell.numberFormat).includes('%')) {
      ...
      if (formula.includes('*100') || formula.includes('* 100')) {
        return createIssue(this, {
          message: `百分比重复转换：${cell.address} 公式中已乘以100，但单元格格式也设置为百分比`,
          cellAddress: cell.address,
          sheetName: context.name,
          formula,
          confidence: ConfidenceLevel.HIGH,
          suggestion:
            "移除公式中的'*100'，保留百分比格式即可。或移除百分比格式，保留*100",
  ...
  checkExcel(cell, context) {
    ...
    if (medianValue > 0) {
      ...
      if (ratio > MAGNITUDE_THRESHOLD || ratio < 1 / MAGNITUDE_THRESHOLD) {
        return createIssue(this, {
          message: `数量级异常：单元格 ${cell.address} 的值 (${cellValue.toLocaleString('en-US', { maximumFractionDigits: 0 })}) 与同列其他值的数量级不一致`,
          cellAddress: cell.address,
          sheetName: context.name,
          confidence: ConfidenceLevel.MEDIUM,
          suggestion: '检查是否存在单位不一致（如万元与元混合），或数据输入错误',
          details: {
